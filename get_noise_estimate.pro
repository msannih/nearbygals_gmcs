...
; NAME:
;
; get_noise_estimate --> hacked from AKL's make_noise_cube
;
; PURPOSE:
;
...
;       COLLAPSE TO TWO DIMENSIONS
        ...
        if n_elements(noise_file) gt 0 then begin           
           writefits, noise_file, noise_cube, twod_head(noise_hdr)
        endif

        meannoise=median(noise_cube,/even)
